...
// void VendingMode::Inner::ReturnChange(u16 change){
//   RB_LOG_DEBUG( "Exchange Money is " << change << "cent" );
//   mMoneyInIntermediateCash = change;
...
void VendingMode::Inner::onCoinsInserted( u16 money )
{
  ...
  if ( isProductValid || ( mVendingState == STATE_WAITING ) )
  {
    ...
    {
      if (mProductSlots[mSelectedProduct].getPricePerItem() < mCashBox.getMoneyInIntermediateCash()){
        startReleaseWorkflow();
        mCashBox.ReturnChange(mProductSlots[mSelectedProduct].getPricePerItem()); //call of our function 
    ...
    else
    {
      if ( t->mProductSlots[t->mSelectedProduct].getPricePerItem() < t->mCashBox.getMoneyInIntermediateCash() )
      {
        t->startReleaseWorkflow();
				VendingScreen::drawReturningChangeMessage();
				
      }else if(t->mProductSlots[t->mSelectedProduct].getPricePerItem() <= t->mCashBox.getMoneyInIntermediateCash()){
				t->startReleaseWorkflow();
			}
      else
      {
